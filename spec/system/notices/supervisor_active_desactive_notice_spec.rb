...
RSpec.describe 'Supervisor activating or deactivating a notice', type: :system do
  let!(:user) { create(:user, :supervisor) }
  let!(:supervisor) { create(:supervisor, user: user) }
  let!(:notice_active) { create(:notice, active: true, user: create(:user, :supervisor)) }
  let!(:notice_inactive) { create(:notice, active: false, user: create(:user, :supervisor)) }
  ...
  context 'when the notice is active' do
    it 'allows the supervisors to deactivate the notice' do
  ...
  context 'when the notice is inactive' do
    it 'allows the supervisors to activate the notice' do
