...
RSpec.describe 'Supervisor creating a notice', type: :system do
  let!(:user_supervisor) { create(:user, :supervisor) }
  let!(:supervisor) { create(:supervisor, user: user_supervisor) }

  before do
    login_as user_supervisor, scope: :user
